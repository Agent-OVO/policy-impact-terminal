...
阶段六“产品边界、事实源、版本体系、有限来源和Token治理封板”已完成。阶段七已经完成可执行数据库迁移、20份官方网页及附件复合原文、确定性投影器、真实影子包、批量装载器、临时PostgreSQL迁移和幂等性验证。阶段八已完成Edge生产Schema类型基线、JSON边界、事务发布/回滚、Token预算硬门、邀请制账户治理、两阶段硬删除和三个管理员专用Edge候选入口。阶段九已完成固定四来源、工作日两次采集、L0—L3确定性分层、24/12/8/3限额、来源健康状态、有限人工队列和重复任务抑制的本地实现，并完成四来源真实只读试跑。阶段十已完成24份报告的统一索引、公司反查、产业聚合、政策事件、关系变化、观察池、通用CLI及0张新表的数据库候选。上述新增能力均尚未部署Supabase，生产读写链保持不变。
...
20. `stage-10-cross-policy-observation-kernel-v1.0.md`
...
- 跨政策观察：全部报告统一生成本地只读索引；产业别名、关系校准事件和观察池保持离线显式契约，数据库候选只复用当前已发布修订投影，不新建重复事实表。
