...
Options:
  ...
  --analysis-limit=<n>        Max L2/L3 items selected for manual analysis per run. Default: ${DEFAULT_ANALYSIS_PER_RUN_LIMIT}.
  --pending-queue-limit=<n>   Max high-value pending queue shown in the artifact. Default: ${DEFAULT_PENDING_QUEUE_LIMIT}.
...
async function fetchNdaPolicyRelease(source, args) {
...
async function ingestCandidates(candidates, args, plan) {
  const { supabaseUrl, accessToken, crawlerSecret } = readIngestEnvironment();
  const pendingQueueKeys = new Set(plan.pendingQueue.map(candidateIdentity));
  ...
  for (const candidate of candidates) {
    ...
    const requiresManualAnalysis = manualAnalysisEligible && pendingQueueKeys.has(identity);
    const analysisQueueSelected = requiresManualAnalysis && analysisQueueKeys.has(identity);
    if (!hasUsableFullText(candidate)) {
...
function printSummary(output, outPath) {
  ...
  console.log(`[summary] layers L0=${output.counts.L0} L1=${output.counts.L1} L2=${output.counts.L2} L3=${output.counts.L3} ingest=${output.counts.ingestSelected} analysis=${output.counts.analysisSelected} overflow=${output.counts.queueOverflow}`);
  ...
  for (const item of output.analysisQueue.slice(0, 8)) {
    console.log(`- ${item.publishDateTime || item.publishDate || "no-date"} ${item.triage.analysisDepth}/${item.triage.reviewPriority} ${item.sourceKey} ${item.title}`);
  }
}
