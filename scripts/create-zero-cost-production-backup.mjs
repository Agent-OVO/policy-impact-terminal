...
const project = process.env.ZERO_COST_BACKUP_TEST_PROJECT_JSON
  ...
  : getProject(args.projectRef);

if (!project) throw new Error(`Supabase project not found: ${args.projectRef}`);
if (project.status !== "ACTIVE_HEALTHY") throw new Error(`Supabase project is not healthy: ${project.status}`);
...
try {
  ...
  const dumpDefinitions = [
    { kind: "roles", file: "roles.sql", args: ["--role-only"] },
    { kind: "schema", file: "schema.sql", args: ["--schema", "public,auth,supabase_migrations", "--keep-comments"] },
    { kind: "data", file: "data.sql", args: ["--data-only", "--use-copy", "--schema", "public,auth,supabase_migrations"] }
  ];
  const files = [];
  ...
  for (const definition of dumpDefinitions) {
    ...
    runSupabase([
      "db", "dump", "--linked", "--password", dbPassword,
      "--file", plainPath, ...definition.args
    ], dbPassword);
    ...
    await fs.writeFile(path.join(backupDir, encryptedName), encrypted.ciphertext);
    files.push({
  ...
  const manifest = {
    ...
    restoreVerificationNote: "A dedicated local restore verification is required before this backup can open the production write gate."
  };
  await writeJson(path.join(backupDir, "manifest.json"), manifest);
  await writeJson(path.join(outputRoot, "latest.json"), {
    backupId,
    manifest: path.relative(outputRoot, path.join(backupDir, "manifest.json"))
  });
  await writeJson(path.join(outputRoot, "latest-manifest.json"), manifest);
...
function getProject(projectRef) {
  const raw = runSupabase(["projects", "list", "--output", "json"], null, true);
  const projects = JSON.parse(raw);
  return projects.find((item) => item.ref === projectRef || item.id === projectRef) ?? null;
}

function runSupabase(commandArgs, secretToMask, capture = false) {
  const base = ["supabase", ...commandArgs];
  const invocation = process.platform === "win32"
    ? { command: process.env.ComSpec || "cmd.exe", args: ["/d", "/s", "/c", ["npx", ...base].map(quoteWindowsArgument).join(" ")] }
...
function redactText(value, explicitSecret = null) {
  ...
  return output.replace(/postgres(?:ql)?:\/\/[^\s]+/gi, "postgresql://[redacted]")
    .replace(/[A-Za-z0-9_-]{40,}/g, "[redacted]").slice(0, 20_000);
}
