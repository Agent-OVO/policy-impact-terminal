...
export function buildLimitedPolicyPlan(candidates, options = {}) {
  ...
  const ingestLimit = toPositiveInteger(options.ingestLimit, 24);
  ...
  const automaticAnalysisSelection = options.automaticAnalysisSelection === true;
  ...
  const recommendedAnalysisQueue = pendingQueue.slice(0, analysisPerRunLimit);
  const analysisQueue = automaticAnalysisSelection ? recommendedAnalysisQueue : [];
  ...
  return {
    ...
    excluded,
    ingestCandidates,
    manualEligibleCandidates: manualEligible,
    pendingQueue,
    recommendedAnalysisQueue,
    analysisQueue,
    ...
    counts: {
      ...
      recommendedAnalysis: recommendedAnalysisQueue.length,
